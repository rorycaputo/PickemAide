...
DRAFTKINGS_ODDS_HEADER = 'Draft Kings Odds'
...
def main():
    ...
    print(build_table(pickem_lines, dk_lines))

def build_table(pickem_lines, dk_lines):
    final_table = []
    for line in pickem_lines:
        dk_team = get_dk_team(line['team'])
    ...
    # Todo the odds sort wont work on 'Even'
    sorted_final_table = sorted(final_table, key=lambda x: (-float(x[DRAFTKINGS_DIFF_HEADER]), float(re.sub(r'[^\d.-]', '', x[DRAFTKINGS_ODDS_HEADER]))))
